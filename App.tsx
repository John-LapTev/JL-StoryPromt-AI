...
const NoteCard: React.FC<{
    ...
    return (
        <div
            className="absolute group glass-panel p-4 rounded-xl flex flex-col cursor-grab board-interactive-item border-l-4 border-l-yellow-500/50"
            style={{
                left: note.position.x,
                top: note.position.y,
                width: note.size.width,
                height: note.size.height,
                background: 'rgba(30, 25, 10, 0.6)', // Warm tint
        ...
        >
            {isEditing ? (
                <textarea
                    ...
                    onMouseDown={(e) => e.stopPropagation()}
                    className="w-full h-full bg-transparent text-yellow-100/90 text-sm resize-none focus:outline-none font-body leading-relaxed"
                />
            ) : (
                <p className="w-full h-full text-yellow-100/90 text-sm whitespace-pre-wrap break-words overflow-hidden pointer-events-none font-body leading-relaxed">
                    {note.text}
                </p>
            )}
            <div
...
const AddFrameMenu: React.FC<AddFrameMenuProps> = ({ targetRect, actions, onClose }) => {
    ...
    return (
        <div ref={menuRef} style={{ top: position.y, left: position.x }} className="absolute z-[60] w-max">
            <div className="flex flex-col gap-1 rounded-xl glass-panel p-1.5 animate-fade-in">
                {actions.map((action, index) => (
                    <button key={index} onClick={action.onClick} className="flex min-w-[180px] cursor-pointer items-center justify-start overflow-hidden rounded-lg h-9 px-3 bg-transparent text-white/90 text-sm font-medium hover:bg-white/10 hover:text-white w-full gap-3 text-left transition-colors">
...
export default function App() {
    ...
    const [timelineViewMode, setTimelineViewMode] = useState<'compact' | 'expanded'>('compact');
    ...
    const updateFrames = useCallback((updater: React.SetStateAction<Frame[]>) => { setLocalFrames(updater); setHasUnsavedChanges(true); }, []);
    const updateAssets = useCallback((updater: React.SetStateAction<Asset[]>) => { setLocalAssets(updater); setHasUnsavedChanges(true); }, []);
    ...
    const handleOpenDossier = useCallback((dossier: ActorDossier) => {
        const related = localFrames.filter(f => f.sourceHash === dossier.sourceHash);
        setSelectedDossierData({ dossier, relatedFrames: related });
    }, [localFrames]);
    ...
    const executeNewProject = useCallback(() => {
        const newProject: Project = { id: crypto.randomUUID(), name: 'Новый проект', frames: [], assets: [], sketches: [], notes: [], dossiers: [], lastModified: Date.now() };
    ...
    const handleNewProject = useCallback(() => {
        if (hasUnsavedChanges) {
            setConfirmationState({ title: 'Создать новый проект?', message: 'Ваш текущий проект имеет несохраненные изменения. Все изменения будут утеряны.', onConfirm: executeNewProject });
        } else {
            executeNewProject();
        }
    ...
    const executeLoadProject = useCallback((id: string) => { setCurrentProjectId(id); projectService.setLastProjectId(id); setHasUnsavedChanges(false); setSelectedAssetIds(new Set()); setIsLoadModalOpen(false); setConfirmationState(null); }, []);
    const handleLoadProject = useCallback((id: string) => { if (hasUnsavedChanges) { setConfirmationState({ title: 'Загрузить проект?', message: 'Ваш текущий проект имеет несохраненные изменения. Все изменения будут утеряны.', onConfirm: () => executeLoadProject(id) }); } else { executeLoadProject(id); } }, [hasUnsavedChanges, executeLoadProject]);
    const handleDeleteProject = (id: string) => {
        if (id === DEMO_PROJECT_ID) { alert("Демонстрационный проект нельзя удалить."); return; }
        ...
        setConfirmationState({
            ...
            onConfirm: () => {
                 ...
                 setProjects(updatedProjects);
                 projectService.saveProjects(updatedProjects);
                 if (currentProjectId === id) { if (updatedProjects.length > 0) { const nextProject = updatedProjects.sort((a,b) => b.lastModified - a.lastModified)[0]; setCurrentProjectId(nextProject.id); projectService.setLastProjectId(nextProject.id); } else { executeNewProject(); } }
                 setConfirmationState(null);
            }
        });
    };
    ...
    const handleGenerateVideo = async (frame: Frame) => {
        if (!isVeoKeySelected && window.aistudio) {
            setConfirmationState({
                title: "Требуется API ключ Veo",
                message: "Для генерации видео требуется API-ключ с доступом к Veo. Хотите выбрать ключ сейчас?",
                onConfirm: async () => {
                    setConfirmationState(null);
                    try { await window.aistudio!.openSelectKey(); setIsVeoKeySelected(true); } catch (e) { alert("Выбор ключа отменен."); return; }
                }
            });
            return;
        }
        try {
            ...
            if (errorMessage.includes("API key") || errorMessage.includes("квота")) { setIsVeoKeySelected(false); }
            alert(`Ошибка при генерации видео: ${errorMessage}`);
        } finally { setGeneratingVideoState(null); }
    };
    
    const handleManageApiKey = async () => { if (window.aistudio) { try { await window.aistudio.openSelectKey(); const hasKey = await window.aistudio.hasSelectedApiKey(); setIsVeoKeySelected(hasKey); alert(hasKey ? "API-ключ успешно выбран." : "Выбор ключа был отменен или не удался."); } catch (error) { alert("Не удалось открыть диалог выбора API-ключа."); } } else { alert("Функция управления ключами не доступна."); } };
    ...
    const handleRegenerateFrame = useCallback(async (frameId: string) => {
        ...
        const dossier = frame.sourceHash ? localDossiers.find(d => d.sourceHash === frame.sourceHash) : null;
        const hasSource = !!frame.sourceHash;
        const isAdaptationRetry = !!(dossier || hasSource);
        const retryMessage = dossier ? `Повторная адаптация (${dossier.roleLabel || 'Персонаж'})...` : (hasSource ? 'Повторная адаптация...' : 'Перегенерация...');
        
        updateFrames(prev => prev.map(f => f.id === frameId ? { ...f, isGenerating: true, generatingMessage: retryMessage, hasError: false } : f));
        ...
        try {
            ...
            const { imageUrl, prompt: newPrompt, analysis } = result;
    ...
    const handleGenerateFrame = async (prompt: string, insertIndex: number) => {
        ...
        const placeholderFrame: Frame = { id: placeholderId, imageUrls: [], activeVersionIndex: 0, prompt: prompt, duration: 3.0, isGenerating: true, generatingMessage: 'Генерация кадра...', aspectRatio: isAspectRatioLocked ? globalAspectRatio : '16:9', };
        updateFrames(prev => { const framesCopy = [...prev]; framesCopy.splice(insertIndex, 0, placeholderFrame); return framesCopy; });
        try {
            const leftContextFrame = insertIndex > 0 ? localFrames[insertIndex - 1] : null;
            const rightContextFrame = localFrames[insertIndex] || null;
            const { imageUrl, prompt: newPrompt } = await generateImageInContext(prompt, leftContextFrame, rightContextFrame);
            const newFile = dataUrlToFile(imageUrl, `generated-frame-${Date.now()}.png`);
            const hash = await calculateFileHash(newFile);
            updateFrames(prev => prev.map(f => { if (f.id === placeholderId) { return { ...f, imageUrls: [imageUrl], activeVersionIndex: 0, prompt: newPrompt, file: newFile, isGenerating: false, generatingMessage: undefined, sourceHash: hash }; } return f; }));
        } catch (error) { updateFrames(prev => prev.map(f => { if (f.id === placeholderId) { return { ...f, isGenerating: false, generatingMessage: 'Ошибка генерации', hasError: true }; } return f; })); }
    };
    ...
    const handleAnalyzeStory = useCallback(async () => { if (localFrames.length < 2) { alert("Нужно как минимум 2 кадра для анализа сюжета."); return; } setIsAnalyzingStory(true); try { const newPrompts = await analyzeStory(localFrames); if (newPrompts.length === localFrames.length) { updateFrames(prev => prev.map((frame, index) => ({ ...frame, prompt: newPrompts[index], isTransition: false, }))); } else { throw new Error("Количество сгенерированных промтов не совпадает с количеством кадров."); } } catch (error) { alert(`Ошибка при анализе сюжета: ${error instanceof Error ? error.message : String(error)}`); } finally { setIsAnalyzingStory(false); } }, [localFrames, updateFrames]);
    const handleGenerateSinglePrompt = useCallback(async (frameId: string) => { const frame = localFrames.find(f => f.id === frameId); if (!frame) return; setGeneratingPromptFrameId(frameId); try { const newPrompt = await generateSinglePrompt(frame, localFrames); updateFrames(prev => prev.map(f => f.id === frameId ? { ...f, prompt: newPrompt, isTransition: false } : f)); } catch (error) { alert(`Ошибка при генерации промта: ${error instanceof Error ? error.message : String(error)}`); } finally { setGeneratingPromptFrameId(null); } }, [localFrames, updateFrames]);
    ...
    const handleCreateStoryFromAssets = async () => {
        const assetsToUse = selectedAssetIds.size > 0 ? localAssets.filter(asset => selectedAssetIds.has(asset.id)) : localAssets;
        if (assetsToUse.length === 0) { alert("Пожалуйста, добавьте или выберите ассеты для создания сюжета."); return; }
        setGeneratingStory(true); setIsAssetLibraryOpen(false);
        const placeholderFrames: Frame[] = Array.from({ length: frameCount }, (_, i) => ({ id: crypto.randomUUID(), imageUrls: [], activeVersionIndex: 0, prompt: '', duration: 3.0, isGenerating: true, generatingMessage: `Ожидание генерации...`, aspectRatio: isAspectRatioLocked ? globalAspectRatio : '16:9', }));
        ...
        try {
            ...
            for await (const update of storyGenerator) {
                if (update.type === 'progress' || update.type === 'frame') {
                    ...
                    if (update.type === 'progress') { updateFrames(prev => prev.map(f => f.id === targetId ? { ...f, generatingMessage: update.message } : f)); } else if (update.type === 'frame') { let newFile: File; let hash = ''; let imageUrls = update.frame.imageUrls; const hasError = !imageUrls || imageUrls.length === 0; if (hasError) { newFile = new File([], `failed-frame-${update.frame.id}.txt`, {type: 'text/plain'}); } else { newFile = dataUrlToFile(imageUrls[0], `story-frame-${update.frame.id}.png`); hash = await calculateFileHash(newFile); } const finalFrame: Frame = { ...update.frame, file: newFile, isGenerating: false, generatingMessage: undefined, aspectRatio: isAspectRatioLocked ? globalAspectRatio : (update.frame.aspectRatio || '16:9'), sourceHash: hash, hasError }; updateFrames(prev => prev.map(f => f.id === targetId ? finalFrame : f)); }
                }
            }
            updateFrames(prev => prev.map(f => { if (placeholderIds.includes(f.id) && f.isGenerating) { return { ...f, isGenerating: false, generatingMessage: "Ошибка генерации", hasError: true }; } return f; }));
        } catch (error) { alert(`Ошибка при создании сюжета: ${error instanceof Error ? error.message : String(error)}`); updateFrames(prev => prev.map(f => { if (placeholderIds.includes(f.id) && f.isGenerating) { return { ...f, isGenerating: false, generatingMessage: "Ошибка", hasError: true }; } return f; })); } finally { setGeneratingStory(false); }
    };
    ...
    const handleAdaptFrameToStory = useCallback(async (targetId: string, instruction?: string) => {
        ...
        if (!frameToAdapt && !sketchToAdapt) { alert("Не удалось найти объект для адаптации."); return; }
        ...
        if (frameToAdapt) {
            ...
            } catch (error) { alert(`Ошибка при адаптации кадра: ${error instanceof Error ? error.message : String(error)}`); updateFrames(prev => prev.map(f => f.id === targetId ? { ...f, isGenerating: false, generatingMessage: 'Ошибка', hasError: true } : f)); }
    ...
    const handleAdaptFrameAspectRatio = useCallback(async (frameId: string) => {
        ...
        if (!frameToAdapt || !frameToAdapt.aspectRatio) { alert("Не удалось найти кадр."); return; }
        ...
        try {
            const { imageUrl: newImageUrl, prompt: newPrompt } = await adaptImageAspectRatio(frameToAdapt, frameToAdapt.aspectRatio);
            const newFile = dataUrlToFile(newImageUrl, `adapted-ar-${frameId}-${Date.now()}.png`);
            updateFrames(prev => prev.map(f => { if (f.id === frameId) { const newImageUrls = [...f.imageUrls, newImageUrl]; return { ...f, imageUrls: newImageUrls, activeVersionIndex: newImageUrls.length - 1, prompt: newPrompt, file: newFile, isGenerating: false, generatingMessage: undefined, sourceHash: f.sourceHash, hasError: false }; } return f; }));
        } catch (error) { alert(`Ошибка при адаптации соотношения сторон: ${error instanceof Error ? error.message : String(error)}`); updateFrames(prev => prev.map(f => f.id === frameId ? { ...f, isGenerating: false, generatingMessage: 'Ошибка', hasError: true } : f)); }
    }, [localFrames, updateFrames]);

    const handleAdaptAllFramesAspectRatio = async () => {
        updateFrames(prev => prev.map(f => ({ ...f, isGenerating: true, generatingMessage: `Ожидание адаптации к ${globalAspectRatio}...`, hasError: false })));
        for (let i = 0; i < localFrames.length; i++) {
            ...
            try {
                const { imageUrl: newImageUrl, prompt: newPrompt } = await adaptImageAspectRatio(frameToAdapt, globalAspectRatio);
                const newFile = dataUrlToFile(newImageUrl, `adapted-ar-all-${frameId}-${Date.now()}.png`);
                updateFrames(prev => prev.map(f => { if (f.id === frameId) { const newImageUrls = [...f.imageUrls, newImageUrl]; return { ...f, imageUrls: newImageUrls, activeVersionIndex: newImageUrls.length - 1, prompt: newPrompt, file: newFile, isGenerating: false, generatingMessage: undefined, aspectRatio: globalAspectRatio, sourceHash: f.sourceHash, hasError: false }; } return f; }));
            } catch (error) { updateFrames(prev => prev.map(f => f.id === frameId ? { ...f, isGenerating: false, generatingMessage: 'Ошибка адаптации', hasError: true } : f)); }
        }
    };
    ...
    const handleReplaceFrame = (frameId: string) => { const i = document.createElement('input'); i.type = 'file'; i.accept = 'image/*'; i.onchange = async (e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) { try { const b = await fileToBase64(f); const hash = await calculateFileHash(f); updateFrames(p => p.map(fr => { if (fr.id === frameId) { const niu = [b]; return { ...fr, imageUrls: niu, activeVersionIndex: 0, file: f, sourceHash: hash, hasError: false }; } return fr; })); } catch (err) { alert("Could not load image file for replacement."); } } }; i.click(); };
    const handleVersionChange = useCallback(async (frameId: string, direction: 'next' | 'prev') => {
        const frameIndex = localFrames.findIndex(f => f.id === frameId); if (frameIndex === -1) return;
        const frame = localFrames[frameIndex];
        const newVersionIndex = direction === 'next' ? Math.min(frame.activeVersionIndex + 1, frame.imageUrls.length - 1) : Math.max(frame.activeVersionIndex - 1, 0);
        if (newVersionIndex === frame.activeVersionIndex) return;
        const newImageUrl = frame.imageUrls[newVersionIndex];
        let newFile: File;
        try { if (newImageUrl.startsWith('data:')) { newFile = dataUrlToFile(newImageUrl, `frame-${frame.id}-v${newVersionIndex}.png`); } else { const blob = await fetchCorsImage(newImageUrl); newFile = new File([blob], `frame-${frame.id}-v${newVersionIndex}.png`, { type: blob.type }); } } catch (e) { newFile = new File([], `failed-frame-${frame.id}.txt`, { type: 'text/plain' }); }
    ...
    const handleStartIntegration = useCallback(async (source: File | string, targetFrameId: string) => { 
        ...
        if (typeof source === 'string') { const a = localAssets.find(as => as.id === source); if (!a) return; s = a; } else { const i = await fileToBase64(source); s = { id: crypto.randomUUID(), imageUrl: i, file: source, name: source.name }; } 
        setIntegrationConfig({ sourceAsset: s, targetFrame: target }); setIsIntegrationModalOpen(true); 
    ...
    const handleStartIntegrationFromFrame = useCallback((sourceFrameId: string, targetFrameId: string) => {
        const sourceFrame = localFrames.find(f => f.id === sourceFrameId); const targetFrame = localFrames.find(f => f.id === targetFrameId);
        if (!sourceFrame || !targetFrame || !sourceFrame.file) return;
        setIntegrationConfig({ sourceAsset: { id: sourceFrame.id, imageUrl: sourceFrame.imageUrls[sourceFrame.activeVersionIndex], file: sourceFrame.file, name: sourceFrame.prompt || `Кадр #${sourceFrame.id.substring(0,4)}` }, targetFrame });
        setIsIntegrationModalOpen(true);
    }, [localFrames]);
    ...
    const handleBoardDoubleClick = (e: React.MouseEvent) => {
        ...
        const x = (e.clientX - rect.left - transform.x) / transform.scale; const y = (e.clientY - rect.top - transform.y) / transform.scale;
        setAdvancedGenerateModalConfig({ mode: 'generate-sketch', position: { x, y } }); setIsAdvancedGenerateModalOpen(true);
    };

    const handleBoardContextMenu = (e: React.MouseEvent) => {
        e.preventDefault(); setSketchContextMenu(null); setNoteContextMenu(null);
        if ((e.target as HTMLElement).closest('.board-interactive-item')) return;
        const board = boardRef.current; if (!board) return;
        const rect = board.getBoundingClientRect();
        const boardX = (e.clientX - rect.left - transform.x) / transform.scale; const boardY = (e.clientY - rect.top - transform.y) / transform.scale;
    ...
    const handleBoardMouseDown = (e: React.MouseEvent) => {
        ...
        if (e.button === 0 && (e.target as HTMLElement).closest('.board-interactive-item')) { return; }
        setContextMenu(null); setBoardContextMenu(null); setSketchContextMenu(null); setNoteContextMenu(null);
    ...
    const handleBoardMouseMove = (e: React.MouseEvent) => {
        if (panState.current.isPanning) {
            e.preventDefault(); const dx = e.clientX - panState.current.startX; const dy = e.clientY - panState.current.startY;
            setTransform(prev => ({ ...prev, x: panState.current.lastX + dx, y: panState.current.lastY + dy }));
        }
        if (draggingInfo) {
            e.preventDefault(); const dx = (e.clientX - draggingInfo.initialMousePos.x) / transform.scale; const dy = (e.clientY - draggingInfo.initialMousePos.y) / transform.scale;
            switch (draggingInfo.type) {
                case 'note-move': { const newX = draggingInfo.initialPosition.x + dx; const newY = draggingInfo.initialPosition.y + dy; updateNotes(prev => prev.map(n => n.id === draggingInfo.id ? { ...n, position: { x: newX, y: newY } } : n)); break; }
                case 'note-resize': { if (draggingInfo.initialSize) { const newWidth = Math.max(120, draggingInfo.initialSize.width + dx); const newHeight = Math.max(100, draggingInfo.initialSize.height + dy); updateNotes(prev => prev.map(n => n.id === draggingInfo.id ? { ...n, size: { width: newWidth, height: newHeight } } : n)); } break; }
            }
        }
    };

    const handleBoardMouseUp = (e: React.MouseEvent) => { if (panState.current.isPanning) { panState.current.isPanning = false; (e.currentTarget as HTMLElement).style.cursor = 'grab'; } if (draggingInfo) { setDraggingInfo(null); setSketchDropTargetIndex(null); setIsAssetLibraryDropTarget(false); } };

    const handleWheel = useCallback((e: WheelEvent) => {
        if (!boardRef.current) return; e.preventDefault();
        const scaleFactor = 1.1; const newScale = e.deltaY < 0 ? Math.min(8, transform.scale * scaleFactor) : Math.max(0.1, transform.scale / scaleFactor);
        if (Math.abs(newScale - transform.scale) < 0.001) return;
        const rect = boardRef.current.getBoundingClientRect(); const mouseX = e.clientX - rect.left; const mouseY = e.clientY - rect.top;
        const newX = mouseX - (mouseX - transform.x) * (newScale / transform.scale); const newY = mouseY - (mouseY - transform.y) * (newScale / transform.scale);
        setTransform({ scale: newScale, x: newX, y: newY });
    }, [transform]);

    useEffect(() => { const board = boardRef.current; if (!board) return; board.addEventListener('wheel', handleWheel, { passive: false }); return () => { board.removeEventListener('wheel', handleWheel); }; }, [handleWheel]);

    const handleResetView = () => { setTransform({ scale: 1, x: 0, y: 0 }); };
    const handleSketchDragStart = (e: React.DragEvent, sketch: Sketch) => { e.stopPropagation(); setDraggingSketchId(sketch.id); setIsAssetLibraryDropTarget(true); const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); const clickOffsetX = (e.clientX - rect.left) / transform.scale; const clickOffsetY = (e.clientY - rect.top) / transform.scale; dragOffset.current = { x: clickOffsetX, y: clickOffsetY }; e.dataTransfer.setData('application/json;type=sketch-id', sketch.id); e.dataTransfer.effectAllowed = 'copyMove'; };
    const handleSketchDragEnd = () => { setDraggingSketchId(null); setSketchDropTargetIndex(null); setIsAssetLibraryDropTarget(false); };
    const handleNoteMouseDown = (e: React.MouseEvent, note: Note) => { if (e.button !== 0) return; e.stopPropagation(); setDraggingInfo({ type: 'note-move', id: note.id, initialMousePos: { x: e.clientX, y: e.clientY }, initialPosition: note.position, }); };
    const handleNoteResizeMouseDown = (e: React.MouseEvent, note: Note) => { if (e.button !== 0) return; e.stopPropagation(); setDraggingInfo({ type: 'note-resize', id: note.id, initialMousePos: { x: e.clientX, y: e.clientY }, initialPosition: note.position, initialSize: note.size, }); };
    const handleBoardDragOver = (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); const isFrame = e.dataTransfer.types.includes('application/json;type=frame-id'); const isAsset = e.dataTransfer.types.includes('application/json;type=asset-ids'); const isSketch = e.dataTransfer.types.includes('application/json;type=sketch-id'); const isFile = e.dataTransfer.types.includes('Files'); if (isFrame || isSketch) { e.dataTransfer.dropEffect = 'move'; } else if (isAsset || isFile) { e.dataTransfer.dropEffect = 'copy'; } };
    const handleBoardDragLeave = (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); };
    ...
    const handleBoardDrop = async (e: React.DragEvent) => {
        ...
        if (frameId) {
            ...
            if (sketch && boardRef.current) { const boardRect = boardRef.current.getBoundingClientRect(); const newX = (e.clientX - boardRect.left - transform.x) / transform.scale - dragOffset.current.x; const newY = (e.clientY - boardRect.top - transform.y) / transform.scale - dragOffset.current.y; updateSketches(prev => prev.map(s => s.id === sketchId ? { ...s, position: { x: newX, y: newY } } : s)); }
        } else if (assetIdsJson) {
            try {
                const assetIds = JSON.parse(assetIdsJson); if (!Array.isArray(assetIds) || !boardRef.current) return;
                ...
                updateSketches(prev => [...prev, ...newSketches]);
            } catch (err) { console.error("Failed to parse dropped asset data on board", err); }
    ...
    const handleSketchContextMenu = (e: React.MouseEvent, sketch: Sketch) => { e.preventDefault(); e.stopPropagation(); setBoardContextMenu(null); setNoteContextMenu(null); setSketchContextMenu({ x: e.clientX, y: e.clientY, sketch }); };
    const handleNoteContextMenu = (e: React.MouseEvent, note: Note) => { e.preventDefault(); e.stopPropagation(); setBoardContextMenu(null); setSketchContextMenu(null); setNoteContextMenu({ x: e.clientX, y: e.clientY, note }); };
    const handleDuplicateSketch = (sketchId: string) => { const sketch = localSketches.find(s => s.id === sketchId); if (!sketch) return; const newSketch: Sketch = { ...sketch, id: crypto.randomUUID(), position: { x: sketch.position.x + 20, y: sketch.position.y + 20 }, }; updateSketches(prev => [...prev, newSketch]); };
    const handleDeleteSketch = (sketchId: string) => { updateSketches(prev => prev.filter(s => s.id !== sketchId)); };
    const handleAddNote = (position: Position) => { const newNote: Note = { id: crypto.randomUUID(), text: 'Новая заметка...', position, size: { width: 220, height: 180 }, }; updateNotes(prev => [...prev, newNote]); };
    const handleDeleteNote = (noteId: string) => { updateNotes(prev => prev.filter(n => n.id !== noteId)); };
    ...
    if (addFrameMenu) {
        const closeAndRun = (fn: () => void) => () => { fn(); handleCloseAddFrameMenu(); };
    ...
    return (
        <div className="relative flex h-screen w-full flex-col group/design-root overflow-hidden select-none font-body text-white bg-[#050608]">
            ...
            <button
                ...
                className="absolute bottom-8 right-8 z-20 flex size-14 items-center justify-center rounded-full bg-gradient-to-r from-primary to-accent text-white shadow-neon-strong hover:shadow-[0_0_30px_rgba(99,102,241,0.8)] transition-all transform hover:scale-105 active:scale-95 border border-white/20"
            ...
            {selectedDossierData && (
                <DossierModal 
                    dossier={selectedDossierData.dossier}
                    relatedFrames={selectedDossierData.relatedFrames}
                    allFrames={localFrames}
                    onClose={() => setSelectedDossierData(null)}
                    onJumpToFrame={(index) => {
                         const targetId = selectedDossierData.relatedFrames[index].id;
